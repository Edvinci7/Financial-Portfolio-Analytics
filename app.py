...
risk_free = st.sidebar.number_input("Risk-free rate (annual, %)", min_value=0.0, max_value=10.0, value=0.0, step=0.25)
...
def fetch_close_prices(tickers, start, end) -> pd.DataFrame:
    ...
    # Get Close (preferred) or Adj Close
...
def annualized_vol(returns: pd.Series, trading_days=252) -> float:
...
def sharpe_ratio(returns: pd.Series, rf_annual: float, trading_days=252) -> float:
...
def plot_equity(equity: pd.Series):
    ...
    fig.update_layout(title="Portfolio Equity Curve (Cumulative Return)", xaxis_title="Date", yaxis_title="Growth of $1")
    return fig
...
# - forward fill gaps for aligned dates (simple, interview-safe)
